...
{
    [SerializeField] private List<UIItem> UICanvas; // Danh sách các UI canvas có sẵn
    ...
    // Mở một UI cụ thể
    public T OpenUI<T>() where T : UIItem
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public T OpenUI<T>(Transform customParent) where T : UIItem
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public void CloseUI<T>(float time) where T :  UIItem
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public void CloseUIDirectly<T>() where T : UIItem
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
    ...
    public bool IsUIOpened<T>() where T : UIItem
    {
        T canvas = GetUI<T>();
    ...
    public T GetUI<T>() where T : UIItem
    {
        return UICanvas.Find(c => c is T) as T;
    }

    // Kích hoạt một UI cụ thể
    public void ActiveUI<T>() where T : UIItem
    {
        T canvas = GetUI<T>();
        if (canvas != null)
        {
